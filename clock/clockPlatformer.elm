...
targetScore = hour * 60 + minute -- Set your target score right here, the number next to this message!
...
clock target = let hour = round(target) // 60
                   minute = round(target) % 60
                   hourangle = degrees (90 - 30 * toFloat hour)
                   minuteangle = degrees (90 - 6 * toFloat minute)
               in group [circle 100 |> filled white,
                         segment (0,0) (fromPolar (60,hourangle))
                            |> traced (solid red)
                       , segment (0,0) (fromPolar (90,minuteangle))
                            |> traced (solid red)]
...
winBanner winPos t = -- Here you can change what the victory banner looks like.
                    group [
                           ...
                           text (Text.style (victoryStyle white) (fromString "DAT TIME THO")) 
...
platforms = [ ((200,60),1),   ((0,120),1),   ((-200,60),1) , ((-400,120),1),
             ((200,180),-1),   ((0,240),-1),   ((-200,180),-1), ((-400,240),-1)
                       ]

-- 'Add' -> Addition, 'Sub' -> Subtraction, 
-- 'Mul' -> Multiplication, 'Div' -> Subtraction

operations = [ (Add,70),(Sub,50),(Add,30),(Add,60),
               (Null,0),(Null,0),(Null,0),(Add,70)]
...
updateScore mario ((xPlat,yPlat),vxPlat) (op,z)
  = if (mario.oldvy == 0 && nearEnough (xPlat,yPlat) (mario.x,mario.y) && mario.operated == False )
      ...
      then case op of
            Add -> if mario.score + toFloat z < 12 * 60 then mario.score + toFloat z else (mario.score + toFloat z) - 12 * 60
            Sub -> if mario.score - toFloat z >= 0 then mario.score - toFloat z else 12 * 60 + (mario.score - toFloat z)
    ...
    collage w' h' <|
      ...
      [
       ...
       rect w h
          ...
          |> moveX (w/2 + width - 50)
       , 
       text (Text.concat [Text.style (scoreStyle white) (fromString ("The time is: ")),
                          Text.style (scoreStyle red)   (timeConversion mario.score)
...
timeConversion score = let hour = round(score) // 60
                           minute = round(score) % 60
                           hourtext = if hour < 10 then "0" ++ toString hour else toString hour
                           mintext = if minute < 10 then "0" ++ toString minute else toString minute
                       in  fromString (hourtext ++ ":" ++ mintext)
